...
const Card = () => {
  ...
  const [, forceRender] = useState(0); // Force re-render when score updates
  ...
  const handleCheckPlayerClick = (clickedIndex) => {
    ...
    TabClick();
    forceRender(prev => prev + 1); // Force re-render so score updates visually
  };
  ...
  return (
    <div className={`card ${animationStage}`}>
      ...
      <p>Score: {scoreRef.current}</p> {/* Score updates instantly */}
    </div>
  );
};
