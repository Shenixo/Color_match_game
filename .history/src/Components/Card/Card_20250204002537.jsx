...
const Card = () => {
  ...
  const [randomNum, setRandomNum] = useState(0); // This will store the correct index after shuffle
  ...
  const startNewRound = () => {
    ...
    setTimeout(() => {
      ...
      setRandomNum(newTargetIndex); // Now we store the correct index in shuffled array
    ...
    <div className={`card ${animationStage}`}>
      {cardArr.map((_, idx) => (
        <button
          ...
          className={idx === randomNum ? "chosen_card" : ""}
