...
const Card = () => {
  ...
  const handleCheckPlayerClick = (clickedIndex) => {
    ...
    TabClick();
    setClickEnabled(false); // Disable clicks after selecting

    setTimeout(() => {
      setRandomNum(Math.floor(Math.random() * 6)); // Pick a new target card
      startNewRound();
    }, 1000);
  };
    ...
    <div className={`card ${animationStage}`}>
      {cardArr.map((_, idx) => (
        <button
          ...
          key={idx}
          disabled={!clickEnabled} // Disables buttons when clicking is not allowed
        />
      ))}
